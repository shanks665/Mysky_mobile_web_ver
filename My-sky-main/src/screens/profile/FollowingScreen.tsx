...
type FollowingRouteProp = RouteProp<ProfileStackParamList, 'Following'>;
type FollowingNavigationProp = StackNavigationProp<ProfileStackParamList, 'Following'>;
  ...
  useEffect(() => {
    const fetchFollowing = async () => {
      try {
        ...
        // フォロー中ユーザーの詳細情報を取得
        const followingPromises = followingIds.map((id: string) => 
  ...
  if (following.length === 0) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.emptyText}>フォロー中のユーザーはいません</Text>
      </View>
    );
  }
